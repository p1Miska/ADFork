 ...
 *
 * Playback does NOT loop within a single ad: after the gif has played through
 * once, the LAST frame is held. The client cuts the whole overlay as soon as
 * the gif finishes (see AdGifClient#effectiveAdDurationMs), so the hold only
 * covers sub-tick rounding gaps.
 */
public class GifAnimation {
    ...
    /** Total playback length of the gif in milliseconds (sum of frame delays). */
    public int totalDurationMs() {
        return totalDurationMs;
    }

    /**
     * Picks the correct frame for a given elapsed time. Plays through ONCE,
     * then holds the last frame (no looping).
     */
    public Identifier frameAt(long elapsedMs) {
        ...
        if (elapsedMs >= totalDurationMs) {
            return frames.get(frames.size() - 1).textureId; // hold last frame
        }
        long t = elapsedMs;
        for (Frame f : frames) {
            if (t < f.delayMs) {
