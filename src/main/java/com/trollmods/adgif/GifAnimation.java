 ...
 *
 * NOTE ON MINECRAFT 1.21.11 / YARN:
 * This project uses Yarn mappings (net.fabricmc:yarn:1.21.11+build.4),
 * which is the proven/stable mapping set for this Minecraft version.
 * Class/method names below (Identifier, MinecraftClient, NativeImage,
 * NativeImageBackedTexture, ResourceManager#getResource, etc.) follow Yarn
 * naming conventions. If a name here is wrong for the exact Yarn build you
 * end up on, check https://maven.fabricmc.net/docs/yarn-1.21.11+build.4/
 * (the generated javadoc) for the current signature.
 */
public class GifAnimation {
    ...
    public static class Frame {
        ...
        Frame(Identifier textureId, int delayMs) {
     ...
     */
    public static GifAnimation load(ResourceManager manager, Identifier location) {
        try {
    ...
    private static GifAnimation decode(InputStream in) throws Exception {
        ...
        try (ImageInputStream iis = ImageIO.createImageInputStream(in)) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                Identifier texId = Identifier.of("adgif", "dynamic/" + label);
    ...
    private static NativeImageBackedTexture createTexture(String label, NativeImage image) {
        ...
        throw new RuntimeException("[adgif] No compatible NativeImageBackedTexture constructor found. "
                + "Check https://maven.fabricmc.net/docs/yarn-1.21.11+build.4/ for the current signature "
                + "and update GifAnimation#createTexture accordingly.");
    }
}
