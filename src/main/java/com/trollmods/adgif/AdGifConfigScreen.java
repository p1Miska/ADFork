...
/**
 * Settings screen opened from Mod Menu (or anywhere else).
 * Values are applied live while dragging and saved to config/adgif.json
 * when the screen is closed (Done button or Esc).
 */
public class AdGifConfigScreen extends Screen {
    ...
    private void addSlider(int x, int y, String label, double min, double max, double step,
        ...
        SliderWidget slider = new SliderWidget(x, y, SLIDER_W, 20, Text.literal(label), initial) {
            ...
            {
                updateMessage(); // set the proper label immediately on construction
            }
    ...
    /** Locale-independent number: drops the trailing ".0" for whole values. */
