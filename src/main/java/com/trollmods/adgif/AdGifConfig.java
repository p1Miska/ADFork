 ...
 * of the client tick, BEFORE vanilla processes key clicks (handleInputEvents)
 * and BEFORE movement/jump input is sampled (KeyboardInput.tick). Clearing
 * keys there means a fresh jump/attack click cannot slip through the same
 * way it could when freezing at the end of the tick.
 *
 * A cooldown after each ad prevents any trigger from instantly restarting
 * the ad (which caused the "silent one-second flash" after the sound ended).
 */
public class AdGifClient implements ClientModInitializer {
    ...
    private void onClientTick(MinecraftClient client) {
        ...
        if (active) {
            ...
            if (elapsed >= durationMs) {
                active = false;
                // Start the post-ad cooldown exactly when the ad ends.
                cooldownUntilMs = System.currentTimeMillis()
                        + (long) (config.cooldownSeconds * 1000);
            }
            // Refresh the health snapshot on every ad tick too, so any damage
            // taken while frozen cannot fire the damage trigger right after
            // the ad ends.
     ...
     *  - clears held-key state (movement/jump/sneak/...),
     *  - drains queued CLICKS (wasPressed) so vanilla's handleInputEvents -
     *    which runs later in this same tick - finds nothing to act on
     *    (no swing animation, no item use, no inventory opening),
     *  - because useKey reads as released, vanilla also interrupts any
     *    in-progress item use (eating, drawing a bow) on its own.
     */
    ...
    private void tryTrigger(double chance) {
        if (active || onCooldown()) {
            return; // ignore triggers during an ad and during the cooldown
        }
